...
from espnet.optimizer.Adan import Adan 
try:
    import wandb
except Exception:
    wandb = None
...
class AbsTask(ABC):
    ...
    def get_parser(cls) -> config_argparse.ArgumentParser:
        ...
        group.add_argument(
            ...
            default=None,
            help="the metrix to measure distance between source domain and target domain",
        )
        group.add_argument(
            "--keep_nbest_models",
            type=int,
        ...
        group.add_argument(
                ...
                nargs="+",
                default=None,
                help="name_map use key to replace value, to help change keyname in pretrained model and make it match with fine-tune model",
            )
        group.add_argument(
        ...
        group.add_argument(
            ...
            type=int_or_none,
            default=96,
            help="used for bucket sampler",
        )
        group.add_argument(
            "--bucket_length_multiplier",
            type=float,
            default=1.1,
            help="used for bucket sampler",
        )
        ...
        group.add_argument(
            "--max_sample_len", # num of sample points
            type=float,
            default=None,
            help="used for bucket sampler",
        )
        ...
        group.add_argument(
            "--min_sample_len", # num of sample points
            type=float,
            default=None,
            help="used for bucket sampler",
        )
        ...
        group.add_argument(
            "--batch_start", # num of sample points
            type=int,
            default=0,
            help="used for bucket sampler",
        )
        ...
        group.add_argument(
            ...
            type=dict,
            default=None,
            help=" used for MDomainDynamicBucketBatchSampler like dict(gssfar:1,gssmid:0.1) key must in [beamfar,beammid,channelfar,channelmid,gssfar,gssmid,near]",
        )
        ...
        #         type=bool,
        #         default=True,
        #         help="when using wavaugment decide to open augmentation",
        #     )
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        if not distributed_option.distributed or distributed_option.dist_rank == 0:
            ...
            # FIXME(kamo): Should we Fuse logging.getLogger()?
            logging.basicConfig(
        ...
        if args.addfixes:
            logging.info(f"(multiple source training)the following suffix are add to the uid before traning:{args.addfixes}")
        ...
        if args.dry_run:
            ...
            if args.init_param:
                if args.name_map:
                    if isinstance(args.name_map,dict):
                        args.name_map = [args.name_map]
                    assert len(args.name_map) == len(args.init_param)
                else:
                    args.name_map = [None]*len(args.init_param)
                for p,name_map in zip(args.init_param,args.name_map):
    ...
    def build_sequence_iter_factory(
        ...
        batch_sampler = build_batch_sampler(
            ...
            bucket_length_multiplier = args.bucket_length_multiplier,
            max_batch_size = args.max_batch_size,
            bucket_boundaries = args.bucket_boundaries,
            bucket_bsize_dynamic = args.bucket_bsize_dynamic,
            bucket_batch_size = args.bucket_batch_size,
            seed = args.seed,
            addfixes = args.addfixes,
            max_sample_len = args.max_sample_len if hasattr(args,"max_sample_len") else None,
            min_sample_len = args.min_sample_len if hasattr(args,"min_sample_len") else None,
        )
        batch_sampler.generate(0)
        batches = list(batch_sampler) #dili: lazy method-only implement generate then generate batch
        ...
        if iter_options.distributed:
            ...
            #domain: make sure in the same id with different fix in the same GPU
            if args.addfixes and iter_options.batch_type=="multiplebucket":
                outbatches = []
                domainnum  = len(args.addfixes.keys())
                for batch in batches:
                    assert len(batch) % domainnum == 0,f"len(batch):{len(batch)},domainnum:{domainnum}"
                    batch_size = len(batch)//domainnum
                    tmpbatch = []
                    for i in range(domainnum):
                        batchcut = batch[i*batch_size:(i+1)*batch_size][rank::world_size]
                        tmpbatch.extend(batchcut)
                    outbatches.append(tuple(tmpbatch))
                batches = outbatches
            else:
    ...
    def build_model_from_file(
        ...
        if model_file is not None:
            ...
            logging.info(f"loading params in {model_file}")
