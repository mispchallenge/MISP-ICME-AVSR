...
class SoundScpReader(collections.abc.Mapping):
    ...
    def __getitem__(self, key):
        ...
        if "@" in wav:
            wavfiles = wav.split("@")
            rate = soundfile.info(wavfiles[0]).samplerate
            array = np.concatenate([soundfile.read(wavfile)[0] for wavfile in wavfiles])
            return rate, array
...
class SoundScpWriter:
    ...
    def __init__(
        ...
        scpfile = scpfile.parent / f"{scpfile.stem}.shape"
        self.format = format
        self.dtype = dtype
    ...
    def __setitem__(self, key: str, value):
        ...
        if self.format == "pt":
            torch.save(torch.from_numpy(signal),wav)
        else:
