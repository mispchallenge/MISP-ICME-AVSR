...
class Speech2Text:
    ...
    def __call__(
        ...
        encode_output = self.asr_model.encode(**batch)
        enc = encode_output[0] # there is only one data be inputed into encoder, but batchsize=beamsiez samples are input to scores
...
def inference(
    ...
    logging.info(speech2text.asr_train_args)
