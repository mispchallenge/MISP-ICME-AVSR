...
def collect_stats(
    ...
    output_dir: Path,#exp/avsr_status/logdir/status
    ngpu: Optional[int],
        ...
        with DatadirWriter(output_dir / mode) as datadir_writer:
            for iiter, (keys, batch) in enumerate(itr, 1):
                ...
                if ngpu <= 1:
                    ...
                    #  data = model.collect_feats(speech=batch["speech"],speech_lengths=batch["speech_lengths"],text=batch["text"],text_lengths=batch["text_lengths"])
                else:
                    # Note that data_parallel can parallelize only "forward()"
                    # tmpbatch = dict(speech=batch["speech"],speech_lengths=batch["speech_lengths"],text=batch["text"],text_lengths=batch["text_lengths"])
                ...
                for key, v in data.items():
                    for i, (uttid, seq) in enumerate(zip(keys, v.cpu().numpy())):
                        ...
                        if f"{key}_lengths" in data:
                            ...
                            if seq.ndim==3:  #seq: ( channel_num, Length, Dim,)
                                seq = seq[:,:length]
                            else:  # seq: (Length, Dim, ...)
                                seq = seq[:length]
                        else:
                            # seq: (Dim, ...) -> (1, Dim, ...)
                            seq = seq[None]
                        if key == "channels_feats":#to global normalize for all channel overall not just normalize channels respectively
                            # seq: channels,T,dims
                            sum_dict[key] += seq.sum(0).sum(0) / seq.shape[0]
                            sq_dict[key] += (seq**2).sum(0).sum(0) / seq.shape[0]
                            count_dict[key] += seq.shape[1]
                        else:# Accumulate value, its square, and count
