...
class NumElementsBatchSampler(AbsSampler):
    def __init__(
        ...
        #1.read shape files
        # each shape file like:
        #    uttA 100,1,1
        #    uttB 201,1,1
        # the list of shape files are turned to:
        # utt2shapes->list of [{uttA:[100,1,1],.uttB:[201,1,1]..}, {} ]
        utt2shapes = [
            load_num_sequence_text(s, loader_type="csv_int") for s in shape_files
        ]
     
        #first_utt2shape->[{uttA:[100,1,1],.uttB:[201,1,1]..}, {} ]
        first_utt2shape = utt2shapes[0]

        #2.check ids
        #s,d is shape file level,if they have same uids
        ...
        #3. Sort samples in ascending order according their shape in the first columns
        # (shape order should be like (Length, Dim))
        # length sorted key set
        keys = sorted(first_utt2shape, key=lambda k: first_utt2shape[k][0])#以shape的第一列进行排序
        if len(keys) == 0:
        ...
        if padding:
            ...
            #[96x96]->feat_dims=96x96
        else:
            feat_dims = None

        #4. Decide batch-sizes
        batch_sizes = []
        current_batch_keys = []
        for key in keys:
            current_batch_keys.append(key)
            # shape: (Length, dim1, dim2, ...)
            if padding: #use the longest utt to caculate sumbins,because padding while make them as long as the longest one
                for d, s in zip(utt2shapes, shape_files):
                    # the first column of the shape file canbe different and the other must be the same
            ...
            else: #don't caculate padding bins
                bins = sum(
        ...
        #5.根据之前的batchsizelist把keys以minibatch给划分出来
        # Set mini-batch
        self.batch_list = []
