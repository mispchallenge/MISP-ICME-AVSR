...
from espnet2.fileio.npy_scp import PtScpReader,Pt2WavScpReader
...
DATA_TYPES = {
    ...
    "v_no_interpolation":dict(
     ...
     "pt":dict(
        func=PtScpReader,
        kwargs=[],
        help="self definite to load torch array by uid"
        "\n\n"
        "   uid a.pt\n"
        "   uid b.pt\n"
        "   ...",
    ...
    "pt2wav":dict(
        func=Pt2WavScpReader,
        kwargs=[],
        help="self definite to load torch array by uid and normalize to float"
        "\n\n"
        "   uid a.pt\n"
        "   uid b.pt\n"
        "   ...",
    ...
    def __getitem__(self, uid: Union[str, int]) -> Tuple[str, Dict[str, np.ndarray]]:
        ...
        for name, loader in self.loader_dict.items():
            try:
                ...
                if not isinstance(
                    value, (np.ndarray, torch.Tensor, str, numbers.Number,dict)
                ):
                    raise TypeError(
                        f"Must be ndarray, torch.Tensor, str or Number or dict: {type(value)}"
                    )
            except Exception:
