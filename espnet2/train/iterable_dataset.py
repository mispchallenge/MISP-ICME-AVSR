...
class select_fuction:
    def __init__(self,getkey=None,scale=1.0) -> None:
        self.getkey = getkey
        self.scale = scale
        
    def __call__(self,value,type="numpy",getkey=None):
        getkey = self.getkey
        scale = self.scale
        if "@" in value:
            if getkey:
                output = torch.cat([torch.load(path)[getkey] for path in value.split("@")])
            else:
                output = torch.cat([torch.load(path) for path in value.split("@")])
...
DATA_TYPES = {
    ...
    "video":select_fuction("data"),
    "v_mask":select_fuction("mask"),
    "pt":select_fuction(),
    "npy": np.load,
    ...
    "pt2wav": lambda x: pt_load(x,scale=2**-15),
    "text": lambda x: x,
}
...
class IterableESPnetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self.droprate_threhold = droprate_threhold
    ...
    def __iter__(self) -> Iterator[Tuple[Union[str, int], Dict[str, np.ndarray]]]:
        ...
        for count, uid in enumerate(uid_iter, 1):
            ...
            for value, (path, name, _type) in zip(values, self.path_name_type_list):
                ...
                array = func(value)
                if name == "droprate":
                    if not self.droprate_threhold == None:
                        if float(value) > self.droprate_threhold:
                            logging.info(f"Skip the utterance {uid} as its the video frame drop is lager than {self.droprate_threhold}")
                            droprate_skip_flag = True
                            break
                    continue

                data[name] = array

            if droprate_skip_flag:
                continue
