...
class SubReporter:
    ...
    def next(self):
        ...
        for key, stats_list in self.stats.items():
            ...
            # if len(stats_list) < self.count:
            #     stats_list = self.stats[key].extend([self.stats[key][-1]]*(self.count-len(stats_list)))
            #     logging.info(f"align {key} list with {self.count-len(stats_list)} items")
    ...
    def log_message(self, start: int = None, end: int = None) -> str: # it is the main log we usually see in train.log to report in 1/20 epoch
        if self._finished:
            raise RuntimeError("Already finished")
        if start is None:
            start = 0
        ...
        for idx, (key2, stats_list) in enumerate(self.stats.items()):
            assert len(stats_list) == self.count, (len(stats_list), self.count)
            # if len(stats_list) < self.count:
            #     stats_list = self.stats[key2].extend([self.stats[key2][-1]]*(self.count-len(stats_list)))
            #     logging.info(f"align {key2} list with {self.count-len(stats_list)} items")
...
class Reporter: #for overall epoches and subreporter just for one epoch
    """Reporter class.
    ...
    def start_epoch(self, key: str, epoch: int = None) -> SubReporter: #check if epoch -1 status exits and return count
