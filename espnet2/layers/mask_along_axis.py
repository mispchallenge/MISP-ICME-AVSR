...
def roll_by_gather(mask,shift_max,shift_pos=None):
        bsize,n_rows, n_cols = mask.shape
        if shift_pos == None:
            shifts = torch.randint(-shift_max,shift_max+1,(bsize,1,1))
        elif shift_pos == "neg":
            shifts = torch.randint(-shift_max,1,(bsize,1,1))
        elif shift_pos == "pos":
            shifts = torch.randint(0,shift_max+1,(bsize,1,1))
        pos_batch_index = (shifts > 0).squeeze()
        neg_batch_index = (shifts < 0).squeeze()
        
        arange1 = torch.arange(n_rows).unsqueeze(1).repeat(bsize,1,n_cols)
        shifts = shifts.repeat(1,n_rows,n_cols)        
        arange2 = (arange1 - shifts) % n_rows
    
        pos_padmask = arange1[pos_batch_index] < arange2[pos_batch_index]
        neg_padmask = arange1[neg_batch_index] > arange2[neg_batch_index]
        
        mask = torch.gather(mask, 1, arange2)
        #padding
        mask[pos_batch_index] = mask[pos_batch_index].masked_fill_(pos_padmask,0.)
        mask[neg_batch_index] = mask[neg_batch_index].masked_fill_(neg_padmask,0.)
        return mask

class ShiftAlongAxis(torch.nn.Module):
    def __init__(
        self,
        shift_range: int = 10,
        shift_pos : str=None,
        shift_orient: str = "row",
    ):
        self.shift_range = shift_range
        self.shift_pos = shift_pos
        self.shift_orient = shift_orient
        super().__init__()

    def forward(self, mask: torch.Tensor,):
        assert mask.dim() == 3 and mask.shape[-1] == mask.shape[-2] #B,T,T
        fnum = mask.shape[-1]
        shift_range = min(fnum,self.shift_range)
        mask = roll_by_gather(mask,shift_range,self.shift_pos)
        if self.shift_orient == "row": #mask x hidden
            return mask
        elif self.shift_orient == "col":
            return mask.transpose(-1,-2) #hidden x mask
