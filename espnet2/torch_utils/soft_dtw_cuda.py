...
def get_avaliable_memory():
    ...
    meminfo = torch.cuda.memory_allocated()
    # print(f"gpu is :{meminfo}")
    print('cuda:is' +' : '+ str(round(meminfo/1024**2)) +' MIB')
    mem = psutil.virtual_memory()
    print('cpu is' +' : '+ str(round(mem.used/1024**2)) +' MIB')
...
def profile(batch_size, seq_len_a, seq_len_b, dims, tol_backward):
    sdtw = SoftDTW(False, gamma=0.01, normalize=False)
    sdtw_cuda = SoftDTW(True, gamma=0.01, normalize=False)
    n_iters = 6
...
if __name__ == "__main__":
    batch_size,seq_len_a,seq_len_b,dims = 16,256,256,80
    sdtw_cuda = SoftDTW(use_cuda=True, gamma=0.01, normalize=False)
    a_cpu = torch.rand((batch_size, seq_len_a, dims))
    b_cpu = torch.rand((batch_size, seq_len_b, dims))
    a_gpu = a_cpu.cuda()
    b_gpu = b_cpu.cuda()
    forward = sdtw_cuda(a_gpu, b_gpu).detach().mean() #(B,) -> (1)
    print(forward)
    ...
    # profile(8,512,512, 80, tol_backward=1e-3)
