...
def load_pretrained_model(
    ...
    if name_map is not None:
        tmp_state = {}
        for src_k,tgt_k in name_map.items():
            for k in src_state.keys():
                if k.startswith(src_k) and k.replace(src_k,"")[0] == ".":  
                    tmp_state[k.replace(src_k,tgt_k)] = src_state[k]
        src_state.update(tmp_state)
    ...
    for key in src_state.keys():
         logging.info(
                    f"loading {key} from pretrained dict"
                )
