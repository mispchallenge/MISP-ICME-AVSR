...
class CrossAttentionFusionEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        """Construct an EncoderLayer object."""
        super(CrossAttentionFusionEncoderLayer, self).__init__()
        self.self_attn = self_attn
        ...
        self.norm_ff = LayerNorm(size)  # for the FNN module
        self.norm_mha = LayerNorm(size)  # for the MHA module
        self.norm_crossmha = LayerNorm(size)
        if feed_forward_macaron is not None:
            self.norm_ff_macaron = LayerNorm(size)
            self.ff_scale = 0.5
        else:
    ...
    def forward(self, x_input, mask, video, video_mask,att_augmask=None,cache=None):
        ...
        if self.src_first:
            ...
            if pos_emb is not None:
                ...
                x_att = self.src_attn(torch.bmm(att_augmask, video) if att_augmask else video, x, x, mask)
