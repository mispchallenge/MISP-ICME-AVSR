...
class CTCPrefixScorer(BatchPartialScorerInterface):
    ...
    def batch_score_partial(self, y, ids, state, x):
        ...
        Args:
            y (torch.Tensor): 1D prefix token [B,T]
            ids (torch.Tensor): torch.int64 next token to score [Beamsize, Beamprefixsize]
            state: decoder state for prefix tokens
            x (torch.Tensor): 2D encoder feature that generates ys [encoder output] [B,D]
