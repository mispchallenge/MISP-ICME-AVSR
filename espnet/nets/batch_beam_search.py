...
class BatchHypothesis(NamedTuple):
    ...
    yseq: torch.Tensor = torch.tensor([])  # (B, T)
    score: torch.Tensor = torch.tensor([])  # (B,)
    length: torch.Tensor = torch.tensor([])  # (B,)
    scores: Dict[str, torch.Tensor] = dict()  # values: (B,)
...
class BatchBeamSearch(BeamSearch):
    ...
    def score_full(
        ...
        for k, d in self.full_scorers.items():
            if k == "decoder" and isinstance(d,AVInDecoder):
                scores[k], states[k] = d.batch_score(hyp.yseq, hyp.states[k], input[0], input[1])
            else:
    ...
    def search(self, running_hyps: BatchHypothesis, x:Union[tuple,torch.Tensor]) -> BatchHypothesis:
        ...
        """
        # hypothesis yseq(B, T),
        # score(dict{"scorername":score of each yseq}),
        # length(B,),
        # score(B,score of each yseq),combine
        # states{"scorername":state,""}
        ...
        if isinstance(input,tuple):
            # full_scorers:{decoder,lm} scores:{"decoder":tensor.shape=(beam_size X vocal_size) } # to find the probility of each sentences's(all beam_size*voca_size sentences) which added next charactor based on beam_size's hyp
            scores, states = self.score_full(running_hyps, (input[0].expand(n_batch, *input[0].shape),input[1].expand(n_batch, *input[1].shape)))
        else:
            scores, states = self.score_full(running_hyps, x.expand(n_batch, *x.shape)) #scores {"xx":score(B,V),"att":score(B,V)}
        ...
        if self.do_pre_beam:
            ...
            part_ids = torch.topk(pre_beam_scores, self.pre_beam_size, dim=-1)[1] #part_ids: (beamsize,pre_beam_size)
