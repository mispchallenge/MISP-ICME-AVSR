...
if __name__ == '__main__':
    ...
    with DatadirWriter(tgtdir) as writer:    
        ...
        if mode != "bpe":
            filedic = func(input)
        for key,value in tqdm(filedic.items()):
            if mode == "text":
                    subwriter[key] = str(len(value))
            elif mode == "pdf":
                pdf_fps= 25
                characters = value['pdf']
                end_timestamp = np.array(value['stamp'])
                subwriter[key] = str(int(np.around(end_timestamp[-1]*pdf_fps)))
            elif mode == "pt_arr":
                if isinstance(value,dict):
                    value = value["data"]
                vstr = ""
                if dimnum > len(value.shape):
                    vstr = "1,"
                    for i in range(dimnum-1):
                        vstr = vstr+str(value.shape[i]) + ","  
                else:
                    for i in range(dimnum):
                        vstr = vstr+str(value.shape[i]) + ","  
                subwriter[key] = vstr[:-1]
            elif mode == "wav":            
                subwriter[key] = str(value[1].shape[0])
            elif mode == "bpe":
                subwriter[key] = str(len(value[0]))+","+str(filedic.token_listnum) #
