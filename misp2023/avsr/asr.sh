...
# ========================== Main stages start from here. ==========================
    ...
    if ! "${skip_data_prep}"; then
        ...
        if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
            log "Stage 0: Data preparation in kaldi data format, etc."
            [[ -n ${self_fix} ]] && ${local_data_opts}+= "--self_fix $self_fix"
            local/gssdata.sh --stage 1 ${local_data_opts}
        fi

        if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
            if [ -n "${speed_perturb_factors}" ]; then
                log "Stage 1: Speed perturbation"
                for dset in gss_train_far ;do # dili:change  
                    _dirs=""
                    if [[ $dset =~ "train" ]] && [[ ! $dset =~ "sp" ]];then
                        if [ ! -e ${dset}_sp ];then #check if exits
                            log "Speed perturbation ing: data/${dset} -> data/${dset}_sp"
                            for factor in ${speed_perturb_factors}; do
                                if [[ $(bc <<<"${factor} != 1.0") == 1 ]]; then
                                    scripts/utils/perturb_data_dir_speed.sh "${factor}" "data/${dset}" "data/${dset}_sp${factor}"
                                    _dirs+="data/${dset}_sp${factor} "
                                else
                            ...
                            log "${dset}_sp exists then skip"
                        fi
                        echo "utils/combine_data.sh "data/${dset}_sp" ${_dirs}"
                        utils/combine_data.sh "data/${dset}_sp" ${_dirs}       
                    fi
                done
        ...
        if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
            if [ "${feats_type}" = raw ]; then
                ...
                for dset in gss_train_far_sp gss_dev_far gss_sum_far; do # dili:change
                    _suf="/org"
                    log "formating "${data_feats}${_suf}/${dset}" "
                    _opts=
                    ...
                    scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" --audio-format "pt" --fs "${fs}" ${_opts} "data/${dset}/wav.scp" "${data_feats}${_suf}/${dset}"           
                done
        ...
        if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
            log "Stage 3: Generate token_list from ${lm_train_text} to ${token_list}"
            cat ${data_feats}/gss_train_far_sp/text  | awk ' { if( NF != 1 ) print $0; } ' > "${data_feats}/lm_train.txt" # dili:change        
        ...
        if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
            # NOTE dili: here we simplify the shape files processes and store them with other .scp file in the same directory
            log "Stage 4: text_shape|speech_shape|text_shape.char"
            for dset in gss_train_far_sp gss_dev_far gss_sum_far; do
                
                if [ -e ${data_feats}/${dset}/utt2num_samples ];then
                    cp "${data_feats}/${dset}/utt2num_samples" "${data_feats}/${dset}/speech_shape"
                    
                else
                    ./local/create_shapefile.sh --nj 32 --input "dump/raw/$tgtdir/wav.scp" --output "dump/raw/$tgtdir/speech_shape" --dimnum 1
                fi

                    python ./local/create_shapefile.py --input "${data_feats}/${dset}/text" --output "${data_feats}/${dset}/text_shape"
                    <"${data_feats}/${dset}/text_shape" \
                    awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
                    >"${data_feats}/${dset}/text_shape.${token_type}"
            done
            
        fi

        if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
            log "stage 5: prepare concat2 diretory and combine it with far_sp"        
            python ./local/utt_concat_train.py --datadir dump/raw/gss_train_far_sp --catdatadir dump/raw/gss_train_far_sp_cat2 --concatnum 2 --maxduration 20
            echo "raw" > $catpath/feats_type
            comebinedir=dump/raw/gss_train_far_sp_cat2
            utils/combine_data.sh --extra-files "speech_shape text_shape.char" "$comebinedir" "dump/raw/gss_train_far_sp $catpath"      
            utils/utt2spk_to_spk2utt.pl $comebinedir/utt2spk > $catpath/spk2utt
            echo "raw" > $comebinedir/feats_type
        fi     
    fi
    
    if "${use_lm}"; then
        if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
            log "Stage 6: LM Training: train_set=${data_feats}/lm_train.txt, dev_set=${data_feats}/lm_dev.txt"
            cat ${data_feats}/gss_dev_far/text  | awk ' { if( NF != 1 ) print $0; } ' > "${data_feats}/lm_dev.txt" # dili:change
            _opts=
            if [ -n "${lm_config}" ]; then
            ...
            if [ "${num_splits_lm}" -gt 1 ]; then
                ...
                _opts+="--train_shape_file ${data_feats}/gss_train_far_sp/text_shape.${lm_token_type} "
            fi
            ...
            ${python} -m espnet2.bin.launch \
                ...
                ${python} -m espnet2.bin.lm_train \
                    ...
                    --valid_shape_file "${data_feats}/gss_dev_far/text_shape.${lm_token_type}" \
    ...
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        ...
        log "Stage 8: ASR Training: train_set=${_asr_train_dir}, valid_set=${_asr_valid_dir}"
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/${_scp},speech,pt "
            ...
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/${_scp},speech,pt "
