...
# ========================== Main stages start from here. ==========================   
    ...
    if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
        _avsr_train_dir="${data_feats}/${train_set}"
        _avsr_valid_dir="${data_feats}/${valid_set}"
        log "Stage 1: AVSR Training: train_set=${_avsr_train_dir}, valid_set=${_avsr_valid_dir}"
        ...
        _feats_type="$(<${_avsr_train_dir}/feats_type)"
        ...
        if [ "${feats_normalize}" = global_mvn ]; then
            ...
            _opts+="--normalize=global_mvn --normalize_conf stats_file=${_avsr_train_dir}/feats_stats.npz "
        fi
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            mkdir -p ${_avsr_train_dir}/split
            _split_dir="${_avsr_train_dir}/split/splits${num_splits_asr}"
            if [ ! -f "${_split_dir}/.done" ]; then
                rm -f "${_split_dir}/.done"
                ${python} -m espnet2.bin.split_scps \
                    --scps \
                        "${_avsr_train_dir}/${_scp}" \
                        "${_avsr_train_dir}/roi.scp" \
                        "${_avsr_train_dir}/text" \
                        "${_avsr_train_dir}/speech_shape" \
                        "${_split_dir}/video_shape" \
                        "${_avsr_train_dir}/text_shape.${token_type}" \
            ...
            _opts+="--train_data_path_and_name_and_type ${_avsr_train_dir}/${_scp},speech,pt "
            _opts+="--train_data_path_and_name_and_type ${_avsr_train_dir}/roi.scp,video,pt "
            _opts+="--train_data_path_and_name_and_type ${_avsr_train_dir}/text,text,text "
            _opts+="--train_shape_file ${_avsr_train_dir}/speech_shape "
            _opts+="--train_shape_file ${_avsr_train_dir}/video_shape "
            _opts+="--train_shape_file ${_avsr_train_dir}/text_shape.${token_type} "
        fi

        log "Generate '${avsr_exp}/run.sh'. You can resume the process from stage 14 using this script"
        mkdir -p "${avsr_exp}"; echo "${run_args} --stage 11 \"\$@\"; exit \$?" > "${avsr_exp}/run.sh"; chmod +x "${avsr_exp}/run.sh" || echo "acl erro skill chmod a+x"
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.avsr_train \
                ...
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                --valid_data_path_and_name_and_type "${_avsr_valid_dir}/${_scp},speech,pt" \
                --valid_data_path_and_name_and_type "${_avsr_valid_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_avsr_valid_dir}/roi.scp,video,pt" \
                --valid_shape_file "${_avsr_valid_dir}/speech_shape" \
                --valid_shape_file "${_avsr_valid_dir}/video_shape" \
                --valid_shape_file "${_avsr_valid_dir}/text_shape.${token_type}" \
                --resume true \
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        log "Stage 1: Decoding: training_dir=${avsr_exp}"
